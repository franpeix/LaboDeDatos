...
df.fillna(0) #los valores nan los reemplaza por 0

values = {"nota1": 0, "nota2": 0, "aprueba": False} #los valores nan los reemplaza por esos valores
...
df.replace({7: 8, 3: 2}) #cambiar todos los valores que aparezcan
...
df.rename(columns={"nota1": "Parcial1", "nota2": "Parcial2"}) #cambiar los nombres de las columnas


#%% calcular promedio y otras cosas

df.agg('min') #para cada columna de devuelve el minimo, donde los strings estan ordenados alfabeticamente
...
df['aprueba'].all() #me fijo si todos aprueban
df['aprueba'].any() #si alguno aprueba
...
df[df['nota1']>=7] #me devuelve las filas de los valores que cumplen la condicion

(df['nota1']>=7).sum()

df[df['nota1']>=7] # nos da el sub-dataframe donde se cumple la condición

df[ (df['nota1']>=7) & (df['nota2']>=7)] #los que cumplen ambas condiciones, para usar un "or" uso |

df[ df['nota1']== 7]


df[ df['nota1'].isin([7,4])] #si pertenece, si la nota es un 7 o un 4

df[(df['nota2'] <=7) & df['aprueba']]

df[(df['nota2'] <=7) | df['aprueba']] #cumple alguna de las condiciones
